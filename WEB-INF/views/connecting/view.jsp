...
<div class="container">
	<div class="row mt-5">
	    <div class="col-lg-12">
	     	<h3 class="page-header" style="margin-top: 100px; color:#2C4964; font-family: 'Gill Sans', 'Gill Sans MT', Calibri, 'Trebuchet MS', sans-serif; font-weight: bold;">커넥팅게시판 > 상세보기</h3>
	    </div>
	  </div>
    <!-- 게시물 상세보기 카드 -->
	  <div class="row mt-3">
	      ...
	      <div class="card border-0 rounded-0">
               <!-- 게시물 상세 정보 -->
			        <div class="card-header bg-white border-light">
					  <div class="row">
					    <div class="col-sm-12 text-left">
					      <div class="row">					      	
					         <div class="col-md-2">
					          <label for="title" class="font-weight-bold" style="color:#2C4964; font-family: 'Gill Sans', 'Gill Sans MT', Calibri, 'Trebuchet MS', sans-serif;">분류</label>
					          <p class="card-text">${cbvo.classification} > ${cbvo.segmentation}</p>
					        </div>
					        <div class="col-md-2">
						 ...
						 <div class="card-body">
						  <div class="form-group">
						   <label for="title" class="font-weight-bold" style="color:#2C4964; font-family: 'Gill Sans', 'Gill Sans MT', Calibri, 'Trebuchet MS', sans-serif;">제목</label>
						    <input type="text" id="title" name="title" class="form-control rounded-30" value="${cbvo.title}" readonly style="background-color: white; color: black;">
						  </div>			
						  <div class="form-group">
						    ...
						    <textarea id="content" name="content" class="form-control rounded-30" rows="3" readonly style="background-color: white; color: black;">${cbvo.content}</textarea>
						  </div>
						</div>
		            </div> 
		           </div>
				...
				<div class="d-flex justify-content-end">
				  <form action="/connecting/modify" method="get" style="margin-right: 5px;">
				  	<input type="hidden" name="${_csrf.parameterName }"   value="${_csrf.token }">
				    <input type="hidden" name="cno" value="${cbvo.cno }">
				     <!-- 로그인한 사용자가 작성한 글 - 수정버튼표시 -->
					<sec:authorize access="isAuthenticated()">
						<sec:authentication property="principal" var="p"/>
							<c:if test="${p.username == cbvo.id }">
				  ...
				  <form action="/connecting/remove" method="post" style="margin-right: 5px;">
				 	 <input type="hidden" name="${_csrf.parameterName }"   value="${_csrf.token }">
				    <input type="hidden" name="cno" value="${cbvo.cno }">
    ...
    <sec:authorize access="isAuthenticated()">
        ...
        <c:set var="isCurrentUserPostOwner" value="${username eq cbvo.id}" scope="request"/>
...
$('#recommend-button').on('click', function() {
    ...
    var writerId = '${cbvo.id}';  // 게시글 작성자 아이디. 실제 코드에 맞게 수정해주세요.
...
function showUploadedFile(result){  
	$(result).each(function(i, obj){  
		...
		if(obj.fileType){	
		    ...
		    liTag += '<img src="/connecting/display?fileName='+ thumbImg +'"><br>' +
...
function goToListPage() {
    var type = '${ccri.type}';
    var keyword = '${ccri.keyword}';
    var pageNum = ${ccri.pageNum};
    var amount = ${ccri.amount};

    location.href = '/connecting/list?type=' + type + '&keyword=' + keyword + '&pageNum=' + pageNum + '&amount=' + amount;
}
...
$('.uploadResult').on('click', 'li img', function(e){
    var li = $(this).parent();
    var filePath = '/connecting/display?fileName='+
...
$(".uploadResult").on('click', '.download-btn', function() {
    ...
    window.location.href = "/connecting/download?fileName=" + filePath;
});
