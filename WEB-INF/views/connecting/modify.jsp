...
<div class="container">
	<div class="row mt-5">
	    <div class="col-lg-12">
	      <h3 class="page-header" style="margin-top: 100px; color:#2C4964; font-family: 'Gill Sans', 'Gill Sans MT', Calibri, 'Trebuchet MS', sans-serif; font-weight: bold;">커넥션게시판 > 수정</h3>
	    </div>
	  </div>
<!-- 폼 이어 붙이기 -->
	 <!-- 게시물 상세보기 카드 -->
		  <div class="row mt-3">
		   <form id="modifyForm" action="/connecting/modify" method="post" role="form">
		    <div class="col-lg-12">
		      <div class="card border-0 rounded-0">
	               <!-- 게시물 상세 정보 -->               
				        <div class="card-header bg-white border-light">
						  <div class="row">
						    <div class="col-sm-12 text-left">
						      <div class="row">
						       <div class="form-group">
								   ...
								   <label for="classification" class="font-weight-bold" style="color:#2C4964; font-family: 'Gill Sans', 'Gill Sans MT', Calibri, 'Trebuchet MS', sans-serif;">분류-1</label>
									<select id="classification" name="classification" class="form-control">
									    <option value="${cbvo.classification}">${cbvo.classification}</option>
									    <option value="강아지">강아지</option>
									    <option value="고양이">고양이</option>
									    <option value="기타">기타</option>
									</select>
									
									<label for="segmentation" class="font-weight-bold" style="color:#2C4964; font-family: 'Gill Sans', 'Gill Sans MT', Calibri, 'Trebuchet MS', sans-serif;">분류-2</label>
									<select id="segmentation" name="segmentation" class="form-control">
									    <!-- 여기에 JavaScript로 선택지를 추가하겠습니다. -->
									</select>
						        ...
						        <div class="col-md-2">
						          ...
						          <p class="card-text"><fmt:formatDate value="${cbvo.regDate}" pattern="yyyy.MM.dd HH:mm (E)"/></p>
						        </div>
						        <div class="col-md-2">
						        ...
						        <div class="col-md-2">
						          ...
						          <p class="card-text"><span id="recommendCnt">${cbvo.recommendCnt}</span></p>
						        </div>
						        </div>
						         </div>
						      </div>
							...
							<div class="form-group">
							    ...
							    <input type="text" id="title" name="title" class="form-control rounded-30" value="${cbvo.title}" style="background-color: white; color: black;">
							...
							<div class="form-group">
							    ...
							    <textarea id="content" name="content" class="form-control rounded-30" rows="3"  style="background-color: white; color: black;">${cbvo.content}</textarea>
					...
					<div class="d-flex justify-content-end">
							<!-- 수정 버튼 -->
							<input type="hidden" name="cno" value="${cbvo.cno}">
							...
							<!-- 삭제 버튼 -->
							<input type="hidden" name="cno" value="${cbvo.cno}">
							<button data-oper="remove" class="btn btn-toss3" style="margin-right: 5px; font-family: 'Gill Sans', 'Gill Sans MT', Calibri, 'Trebuchet MS', sans-serif; font-weight: bold;" formaction="/connecting/remove">
								삭제
							</button>
						
							<!-- 목록 버튼 -->
							<button data-oper="list" class="btn btn-toss4" style="font-family: 'Gill Sans', 'Gill Sans MT', Calibri, 'Trebuchet MS', sans-serif; font-weight: bold;"formaction="/connecting/list"
								 	formmethod="get">
								목록
							</button>
						</div>
	                	...
	                	<input type="hidden" name="type" 	value="${ccri.type}">
	                	<input type="hidden" name="keyword" value="${ccri.keyword}">
						<input type="hidden" name="pageNum" value="${ccri.pageNum}">
	                	<input type="hidden" name="amount"  value="${ccri.amount}">
						<input type="hidden" name="${_csrf.parameterName }" 
										     value="${_csrf.token }">
						<input type="hidden" name="cno" value="${cbvo.cno}">
						<input type="hidden" name="uploadedFiles" value="">	
						<input type="hidden" name="id" value="${cbvo.id}">
...
<script>
//페이지가 로드되었을 때 저장된 값을 기본값으로 설정
document.addEventListener('DOMContentLoaded', function() {
    var classification = document.getElementById('classification').value;
    var segmentation = document.getElementById('segmentation');
    createOptions(classification, segmentation);
    segmentation.value = '${cbvo.segmentation}';
});

// classification이 변경되었을 때 segmentation의 선택지를 업데이트
document.getElementById('classification').addEventListener('change', function() {
    var segmentation = document.getElementById('segmentation');
    createOptions(this.value, segmentation);
});

// 선택지를 생성하는 함수
function createOptions(classification, segmentation) {
    while(segmentation.firstChild) {
        segmentation.removeChild(segmentation.firstChild);
    }
    switch(classification) {
        case '강아지':
            ['치와와', '진돗개', '도베르만','기타'].forEach(function(value) {
                var option = document.createElement('option');
                option.text = value;
                option.value = value;
                segmentation.appendChild(option);
            });
            break;
        case '고양이':
            ['러시안블루', '아비시니안', '페르시안','기타'].forEach(function(value) {
                var option = document.createElement('option');
                option.text = value;
                option.value = value;
                segmentation.appendChild(option);
...
//첨부 파일 목록 가져오기 ----------------------------
(function(){ 
	$.getJSON('/connecting/cnAttachList', { cno : cnoVal }, function(result){
		console.log(result);
...
function showUploadedFile(result) {  
    ...
    $(result).each(function(i, obj) {  
        ...
        if(obj.fileType){
            ...
            liTag += '<img src="/connecting/display?fileName='+ thumbImg +'"></a><br>' +
                       	obj.fileName+
...
$('button[data-oper="modify"]').on('click', function(e) {
    ...
    $('.uploadResult ul li').each(function(i) {
        ...
        formData.append("cnAttachList[" + i + "].uuid", uuid);
        formData.append("cnAttachList[" + i + "].uploadPath", uploadPath);
        formData.append("cnAttachList[" + i + "].fileName", fileName);
        formData.append("cnAttachList[" + i+ "].fileType", fileType ? 'true' : 'false');
    });

    $.ajax({
         url: '/connecting/modify/'+ id,
         type: 'POST',
         processData: false,
         contentType: false,
         data: formData,
         ...
         success : function() {
             ...
             location.href='/connecting/view?cno=' + cnoVal; // 성공해
         },
