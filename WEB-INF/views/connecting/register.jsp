...
<div class="container">
	<div class="row mt-5">
	    <div class="col-lg-12">
	      <h3 class="page-header" style="margin-top: 100px; color:#2C4964; font-family: 'Gill Sans', 'Gill Sans MT', Calibri, 'Trebuchet MS', sans-serif; font-weight: bold;">커넥팅게시판 > 등록</h3>
	    </div>
	  </div>
<!-- 폼 이어 붙이기 -->
	 <!-- 게시물 상세보기 카드 -->
		  <div class="row mt-3">
		   <form action="/connecting/register" method="post" role="form"> 
...
var csrfTokenValue = '${_csrf.token}';		//
	...
	$('button[type="submit"]').on('click', function(e){
	    ...
	    $('.uploadResult ul li').each(function(i, obj){
	        hiddenTag += '<input type="hidden" name="cnAttachList[' + i + '].uuid" ' + 
	                           ' value="' + $(obj).data('uuid') + '">';
	                           
	        hiddenTag += '<input type="hidden" name="cnAttachList[' + i + '].uploadPath" ' + 
	                           ' value="' + $(obj).data('uploadph') + '">';
	       
	        hiddenTag += '<input type="hidden" name="cnAttachList[' + i + '].fileName" ' + 
	                           ' value="' + $(obj).data('filenm') + '">';
	       
	        hiddenTag += '<input type="hidden" name="cnAttachList[' + i + '].fileType" ' + 
	...
	$('input[type="file"]').on('change', function() {
		...
		for (var i = 0; i < files.length; i++) {
		 ...
		 $.ajax({
		        type : 'post',
		        url : '/connecting/upload/ajaxAction',
		        data : formData,
		        dataType : 'json',
		        contentType : false,
		        processData : false,
