...
<div class="container">
 ...
 <div class="section-title">
     ...
     <div class="img">
            <label for="byImg">사진*</label>
            <input type="file" name="byImg" class="form-control" >
           <img src="../resources/img/medipaw/${bvo.byImg }" id="byImg">
           <button type="button" class="btn btn-danger btn-circle" data-file="${bvo.byImg }" 
	      		 data-type="image">  <i class="fa fa-times"></i>　삭제</button>

        </div>
   <form method="post"  action="/boonyang/modify?byno=${bvo.byno }" role="form">
   <div>
    <label for="id">작성자</label>
   <input type="text" name="id" class="form-control" 
					value='${bvo.id }' readonly style="background-color: lightgray;"></div>
      <div>
            <label for="title">제목*</label>
            <input type="text" id="title" name="title" class="form-control" value='${bvo.title }' required>
        </div>
        <div>
            <label for="kind">동물종* </label>
            <input type="text" id="kind" name="kind" class="form-control" value='${bvo.kind }' required>
        </div>
        	<label for="gender">성별*</label>
<input type="radio" name="gender" value="암컷" required
       <c:if test="${bvo.gender == '암컷'}">checked</c:if>>암컷
<input type="radio" name="gender" value="수컷" required
       <c:if test="${bvo.gender == '수컷'}">checked</c:if>>수컷

        <div>
   <div>
    <label for="age">나이*</label>　
    <input type="number" id="age" name="age" class="form-control" style="width: 100px; display: inline-block;" value='${bvo.age }' required > 세
</div>
         ...
         <div>
			...
			<textarea name="content" class="form-control"
							  rows="3" required>${bvo.content }</textarea></div>
       <div>
    <label for="state">분양상태* </label>
    <input type="radio" name="state" value="분양중" required
        <c:if test="${bvo.state == '분양중'}">checked</c:if>>분양중
    <input type="radio" name="state" value="분양완료" required
        <c:if test="${bvo.state == '분양완료'}">checked</c:if>>분양완료
</div>
    ...
    <div class="form-group row mt-4">
        <div class="col text-center">
            <button type="submit" class="btn btn-info regBtn" style="width:70px;">수정</button>
        </div>
        <div class="col text-center">
   ...
   <input type="hidden"  id="img"  name="byImg" value="${bvo.byImg }"/>
    </form>
    </div>
...
$('button[type="submit"]').on('click', function(e) {
    ...
    if (isValid) {
        ...
        swal("*필수입력란을 모두 작성해주세요" ,{icon:'info'});
    }
});
