...
<div class="container">
 ...
 <div class="section-title">
     ...
     <div>
            ...
            <input type="file" name="img" class="form-control" id="imgg" required>
            ...
            <input type="password" id="spw2" name="spw2"class="form-control" required>
             <span id="passwordMatchResult" style="color: red;"></span>
       		...
       		<br>
					...
					<input type="hidden" name="semail" id="semail" required>
					<div class="col-sm-3">
...
<label for="agree">
 ...
 <div class="terms__content">
                ...
                커뮤니티 보기 등 대부분의 메디파우 서비스를 회원과 동일하게 이용할 수 있습니다. 이용자가 메일, 캘린더, 카페,
                블로그 등과 같이 개인화 혹은 회원제 서비스를 이용하기 위해 회원가입을 할 경우, 메디파우는 서비스 이용을
...
function idCheck() {
    $.ajax({
        ...
        success: function(result) {
            if (result == 1) {
                swal("이미 존재하는 아이디입니다.",{icon:'info'});
            } else if (result == 0) {
                $("#idCheckBtn").attr("value", "Y");
                swal("사용 가능한 아이디입니다.",{icon:'info'});
            }
        }
    });
}
...
function sjoinChk() {
	if($("#sname").val() === "" || $("#semail").val() === "" || $("#imgg").val() === "") {
	    swal("필수항목을 모두 입력해주세요.",{icon:'info'});
	    return false;
	}

	// 비밀번호 일치 여부를 확인
    if (!checkPasswordMatch()) {
    	swal("비밀번호가 일치하지 않습니다!",{icon:'info'})
    ...
    if (idCheckBtn.val() === "N") {
       swal("아이디 중복확인은 필수입니다!",{icon:'info'})
        return false;
    }
	//사업자번호 상태조회 여부 체크하기
    ...
    if (cNumCheckBtn.val() === "N") {
       swal("사업자 상태 조회는 필수입니다!",{icon:'info'})
        return false;
    }
	//아이디
	...
	if (sid.length < 5 || sid.length > 10) {
		swal("아이디는 5자에서 10자 사이로 입력하세요.",{icon:'info'});
		return false;
	}
	...
	if (!passwordPattern.test(spw)) {
		swal("비밀번호는 알파벳, 숫자, 특수문자를 포함 8글자 이상이어야 합니다.",{icon:'info'});
		console.log(spw);
		return false;
	}
