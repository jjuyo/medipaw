      ...
      <div class="container">
        ...
        <form action="/reserv/register" method="post" role="form" id="regFrm">
          ...
          <div class="row">
            ...
            <input type="hidden" name="hno" id="hno" value="${param.hno}">
...
$(document).ready(function () {
	// 예약 일자 변경 시 예약 가능한 시간 목록 업데이트
  	$('#rvDate').on('change', function () {
  		var selectDate = new Date($(this).val());			// 선택한 날짜 -> 비교하려면 new Date 객체 사용
        var currentDate = new Date(); 						// 현재 날짜

        if (selectDate <= currentDate) {					// 선택한 날짜가 현재 날짜보다 이전이면 메시지 표시
        	swal("이전 일자는 선택할 수 없습니다.", {
                icon: "warning",
            });
            $(this).val(''); 								// 입력 값을 지워서 무효화
        } else {
        	var selectedDate = $(this).val();				// 선택한 날짜 -> rvDate에 넣으려면 String으로
      	    var hnoVal = $('#hno').val();
